...
async def create_tax_exemption(session: AsyncSession, tax_exemption: TaxExemptionCreate) -> TaxExemption:
    """
...
async def get_tax_exemption_by_id(session: AsyncSession, tax_exemption_id: int) -> TaxExemption | None:
    """
    ...
    """
    result = await session.execute(select(TaxExemption).where(TaxExemption.id == tax_exemption_id))
    return result.scalar_one_or_none()


async def get_tax_exemption_by_code(session: AsyncSession, code: str) -> TaxExemption | None:
    """
    ...
    Returns:
        Tax exemption if found, None otherwise
    """
    result = await session.execute(select(TaxExemption).where(TaxExemption.code == code))
    return result.scalar_one_or_none()


async def update_tax_exemption(
    session: AsyncSession, tax_exemption: TaxExemption, tax_exemption_update: TaxExemptionUpdate
) -> TaxExemption | None:
    """
...
async def delete_tax_exemption(session: AsyncSession, tax_exemption: TaxExemption) -> None:
    """
